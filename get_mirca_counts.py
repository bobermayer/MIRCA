        ...
        for i in range(nexons):
            for k in range(exon_length[i]-K+1):
                if options.use_TC:
                    ...
                    kmer_cov.append([seqs[i][k:k+K]]+[1]+[i,k]+\
                                        [covs[i][max(k-E,0):min(k+K+E,exon_length[i]),n].sum()\
                                             for n in range(nB)])
                else:
                    # use mean read count over stretch of length K+2*E
                    kmer_cov.append([seqs[i][k:k+K]]+[1]+[i,k]+\
                                        [covs[i][max(k-E,0):min(k+K+E,exon_length[i]),n].mean()\
                                             for n in range(nB)])

            kmer_cov.append(['tot',exon_length[i],i,0]+list(covs[i].sum(axis=0)))
