    ...
    for name,chrom,strand,regions in get_regions(inf, options.flank_len):
        ...
        if use_motifs:
            ...
            for motif,vals in tot_motif_cons.iterrows():
                outf.write("{0}\t{1}\t{2:.4f}\t{3:.4f}\n".format(name,motif,vals['count']*overlap_factor[motif],vals['cons']/vals['count']))
        else:
            overlap_factor=tot_kmer_cons[['exon','pos']].groupby('kmer').aggregate(get_overlap_factor)['pos']
            tot_kmer_cons=tot_kmer_cons.groupby('kmer').sum()
            overlap_factor.loc['tot']=1
            for kmer,vals in tot_kmer_cons.iterrows():
                outf.write("{0}\t{1}\t{2:.4f}\t{3:.4f}\n".format(name,kmer,vals['count']*overlap_factor[kmer],vals['cons']/vals['count']))
